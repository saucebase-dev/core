...
namespace Saucebase\Core\Tests\Inertia;
...
class InertiaSSRTest extends TestCase
{
    ...
    /**
     * The macro and the middleware pull in opposite directions, and the order decides.
     *
     * HandleInertiaRequests turns SSR off at the start of every request; the macro
     * turns it back on from the controller. Each is covered alone above — this is the
     * only test that runs both across a live request, which is the arrangement a real
     * page actually uses.
     */
    public function test_a_controller_opting_in_survives_the_middleware_default(): void
    {
        $this->assertTrue(config('inertia.ssr.enabled'), 'SSR should be enabled in config at boot');

        Route::middleware('web')->get('/ssr-opt-in', fn () => Inertia::render('Index')->withSSR());

        $this->get('/ssr-opt-in')->assertOk();

        $this->assertTrue(
            config('inertia.ssr.enabled'),
            'A controller calling ->withSSR() should win over the middleware default.',
        );
    }
