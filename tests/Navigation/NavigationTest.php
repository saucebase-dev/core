...
namespace Saucebase\Core\Tests\Navigation;
...
class NavigationTest extends TestCase
{
    ...
    public function test_load_loads_the_application_navigation_file(): void
    {
        // load() reads routes/navigation.php from the application, which does not
        // exist in the test skeleton, so write one for the duration of the test.
        NavigationFacade::clearResolvedInstances();
        app()->instance(Navigation::class, $this->navigation);

        $path = base_path('routes/navigation.php');
        ...
        Navigation::add('Dashboard', '/dashboard', function (Section $section) {
            $section->attributes(['group' => 'main']);
        });
        PHP);
        ...
        try {
            ...
            $this->assertArrayHasKey('main', $grouped);
            $this->assertSame('Dashboard', $grouped['main'][0]['title']);
        } finally {
            unlink($path);
        }
    }

    public function test_load_is_a_no_op_when_the_application_has_no_navigation_file(): void
    {
        $this->assertFileDoesNotExist(base_path('routes/navigation.php'));

        $this->navigation->load();

        $this->assertSame([], $this->navigation->treeGrouped());
    }
