...
/*
 * Saucebase's export settings for kkomelin/laravel-translatable-string-exporter.
 *
 * Shipped in full rather than as a handful of overrides, because the package only
 * publishes this file — it never calls mergeConfigFrom(). Set individual keys and
 * every key you did not set resolves to null, silently.
 *
 * Merged by ConfigServiceProvider, so an application that publishes its own copy
 * still wins.
 */
return [
